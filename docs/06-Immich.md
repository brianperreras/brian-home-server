...
Official guide: [Immich Docker Compose install](https://docs.immich.app/install/docker-compose)
...
Original photo/video library -> IronWolf 6 TB (/mnt/user/photos/immich-library)
```

## Prerequisites on Unraid

1. Array started with IronWolf as disk 1 and GM7000 as the `fast`/`cache` pool.
2. Shares created: `appdata` on SSD, `photos` on IronWolf ([`04-Storage.md`](04-Storage.md)).
3. Docker enabled with appdata on the SSD ([`05-Docker.md`](05-Docker.md)).
4. Compose Manager (or equivalent) available.
...
The PostgreSQL directory must be local storage on the GM7000 pool, not SMB/NFS and not the IronWolf share.
...
Use Tailscale first. Do not expose port 2283 directly to the public internet. A reverse proxy can be added later after backups, TLS, authentication, and update procedures are established.
...
3. Re-download the current official compose, environment example, and hwaccel file from the latest release download URLs above.
